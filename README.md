...
Full-refresh build → find the run by `QUERY_TAG` → re-run to watch the `merge` upsert.
...
The second build merges `denormalized_data_merge` on `payment_id` (UPDATE matched, INSERT new) —
